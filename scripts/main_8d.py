"""Main experiment with UMAP 8D — does higher-dim embedding improve F1?"""
...
emb = np.load(CACHE / 'umap_152k_21x32_md1.0_8d.npy')   # 8-D!
...
print(f'Vocs: {len(vocs)}, bats: {len(all_bats)}, embedding dim: {emb.shape[1]}')
...
def fit_dp(X, s):
    return BayesianGaussianMixture(n_components=15,
        weight_concentration_prior_type='dirichlet_process',
        weight_concentration_prior=0.1, covariance_type='full',
        max_iter=150, random_state=s).fit(X)
def fit_hdb(X, s):
    ...
    for k in sorted(set(h.labels_)):
        ...
        if m.sum() < 5: continue
        Xi = X[m]; mu = Xi.mean(0)
        Sg = np.cov(Xi.T) if Xi.shape[0]>1 else np.eye(X.shape[1])*0.1
...
df.to_csv('docs/thesis/figures/percontext_152k_21x32_8d_results.csv', index=False)
print('Saved: docs/thesis/figures/percontext_152k_21x32_8d_results.csv')
